...
class managerController extends Controller
{
    ...
    public function index()
    {
        ...
        $datarole = DB::table('roles')->join('users', 'users.role', '=', 'roles.id')->where('users.id', $userid)->get('roles.id')->first();
        $institute_data = DB::table('institutes')->join('instituteusers', 'instituteusers.Inid', '=', 'institutes.Inid')->join('users', 'instituteusers.Uid', '=', 'users.id')->join('citys', 'citys.id', '=', 'institutes.city')->where('instituteusers.Uid', $userid)->get()->first();

        session()->put('Inid', $institute_data->Inid);

        session()->put('roleid', $datarole->id);

        $institute_license = DB::table('license')
            ->where('Inid', $institute_data->Inid)
            ->get('expire_date')
            ->first();

        $isa = DB::table('license_i_s_a')
            ->where('Inid', $institute_data->Inid)
            ->get('expire_date')
            ->first();
        ...
        $data_per = DB::table('permissions')
            ->join('role_permissions', 'role_permissions.permission_id', '=', 'permissions.id')
            ->where('role_permissions.role_id', $datarole->id)
            ->get('slag')
            ->all();

        $departments = DB::table('departments')
            ->join('inst_dep_list', 'departments.Did', '=', 'inst_dep_list.Did')
            ->where('inst_dep_list.Inid', $institute_data->Inid)
            ->select('department_name', 'Semester_no')
            ->get();

        $data = '';

        session()->put('institutedata', $institute_data);
        $student = Student::count();
        // return $student;
        $active_student = Student::where('status', 2)->count();
        // for testing
        // return $departments;
    ...
    public function change_photo(Request $req){
        ...
        $req->validate([
            ...
            'change_photo'=>'required|mimes:jpeg,jpg,png,gif|max:2048'
